...
const PromptDetail: React.FC<PromptDetailProps> = ({ prompt, onBack, onEdit, onDelete, onToggleFavorite, isAuthenticated, onLogin, onTagClick, onAuthorClick, onTopicClick }) => {
  ...
  return (
    <article className="h-full flex flex-col bg-zinc-50/50 dark:bg-zinc-950/50 relative overflow-hidden">
      {jsonLd && createPortal(<script type="application/ld+json" dangerouslySetInnerHTML={{ __html: jsonLd }} />, document.head)}
        ...
        <div className="lg:hidden shrink-0 px-6 pt-6 pb-4 bg-zinc-50/95 dark:bg-zinc-950/95 border-b border-zinc-200/50 dark:border-zinc-800/50 z-20 backdrop-blur-sm sticky top-0">
             <div className="flex items-center justify-between">
                 ...
                 <div className="flex items-center gap-2">
                    <button 
                        onClick={handleShare}
                        className={`p-2 rounded-full transition-colors ${linkCopied ? 'text-green-600 bg-green-50 dark:bg-green-900/20' : 'text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}
                    >
                        {linkCopied ? <RiCheckLine size={20} /> : <RiShareLine size={20} />}
                    </button>
                    <button 
                ...
                <div className="hidden lg:block shrink-0 px-10 pt-10 pb-6 border-b border-zinc-200/50 dark:border-zinc-800/50 bg-zinc-50/50 dark:bg-zinc-950/50 z-10">
                    <div className="flex items-center justify-between mb-6">
                         ...
                         <div className="flex items-center gap-3">
                            <button 
                                onClick={handleShare}
                                className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${linkCopied ? 'bg-green-50 dark:bg-green-900/20 text-green-700 border border-green-200' : 'bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-700'}`}
                                title="Copy direct link"
                            >
                                {linkCopied ? <RiCheckLine size={16} /> : <RiShareLine size={16} />}
                            ...
                            {isAuthenticated && (
                                <>
                                    ...
                                    <button 
                                        onClick={() => onEdit(prompt)}
                                        className="flex items-center gap-2 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-200 hover:bg-zinc-50 dark:hover:bg-zinc-700 px-4 py-1.5 rounded-lg text-sm font-medium shadow-sm"
                                    >
                    ...
                    <div>
                        <div className="flex flex-wrap items-center gap-3 mb-4">
                            ...
                            {prompt.topic && (
                                <button onClick={() => onTopicClick && onTopicClick(prompt.topic!)} className="px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-wider bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700 flex items-center gap-1">
                                     <RiBookOpenLine size={12} /> {prompt.topic}
                                </button>
                            )}
                            {renderStatusBadge(prompt.status)}
                            <span className="text-xs text-zinc-500 flex items-center gap-1">
                                <RiTimeLine size={12} /> Updated {new Date(prompt.updatedAt).toLocaleDateString()}
                            </span>
                        </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 md:p-10 pt-4 md:pt-8 pb-[calc(8rem+env(safe-area-inset-bottom))] lg:pb-10 space-y-8 no-scrollbar">
                    {prompt.imageUrl && (
                        <div className="rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800 shadow-sm bg-zinc-100 dark:bg-zinc-900">
                            <img src={getOptimizedImageUrl(prompt.imageUrl, 1200)} alt={`${prompt.title} Case Example`} className="w-full h-auto block" loading="lazy" decoding="async" onError={handleImageError} />
                        </div>
                    )}

                    {isLocked ? (
                        <LockedPlaceholder title="Private Description" message="The description and context are restricted." />
                    ) : prompt.description ? (
                        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 md:p-8 shadow-sm">
                            ...
                            <Suspense fallback={<div className="space-y-3 animate-pulse"><div className="h-4 bg-zinc-100 dark:bg-zinc-800 rounded w-3/4"></div><div className="h-4 bg-zinc-100 dark:bg-zinc-800 rounded w-full"></div></div>}>
                    ...
                    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 shadow-sm">
                         ...
                         <div className="flex flex-wrap gap-2">
                                {prompt.tags.length > 0 ? (
                                    prompt.tags.map(tag => (
                                        <button key={tag} onClick={() => onTagClick && onTagClick(tag)} className="bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-300 text-xs px-3 py-1.5 rounded-lg font-medium hover:bg-zinc-100 transition-colors">#{tag}</button>
                                    ))
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-3 gap-6">
                            {prompt.author && (
                                <div className="flex flex-col gap-1.5">
                                    <span className="text-xs text-zinc-500 uppercase font-semibold">Author</span>
                                    <button onClick={() => onAuthorClick && onAuthorClick(prompt.author!)} className="text-sm font-medium text-zinc-900 dark:text-zinc-100 hover:underline decoration-blue-300 transition-colors text-left">{prompt.author}</button>
                                </div>
                            )}
                            <div className="flex flex-col gap-1.5">
                                <span className="text-xs text-zinc-500 uppercase font-semibold">License</span>
                                <span className="text-sm font-medium text-zinc-900 dark:text-zinc-100">{prompt.copyright || 'None'}</span>
                            </div>
                            <div className="flex flex-col gap-1.5">
                                <span className="text-xs text-zinc-500 uppercase font-semibold">Version History</span>
                                <span className="text-sm font-medium text-zinc-900 dark:text-zinc-100">{prompt.versions.length} versions</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col h-full">
                    <div className="flex items-center justify-between mb-3 gap-3 shrink-0">
                        <h3 className="text-sm font-semibold text-zinc-900 dark:text-white flex items-center gap-2">
                            <div className="w-2 h-2 bg-zinc-900 dark:bg-white rounded-full"></div> Prompt Content
                        </h3>
                        {!isLocked && (
                            <div className="flex items-center gap-0.5 bg-white dark:bg-zinc-900 p-0.5 rounded-lg border border-zinc-200 dark:border-zinc-800 shadow-sm">
                                <div className="relative flex items-center">
                                    <select value={selectedVersionId} onChange={(e) => setSelectedVersionId(e.target.value)} className="bg-transparent text-xs font-medium text-zinc-600 dark:text-zinc-300 py-1 pl-2 pr-6 outline-none cursor-pointer appearance-none">
                                        {sortedVersions.map((v) => (
                                            <option key={v.id} value={v.id} className="bg-white dark:bg-zinc-900">v{prompt.versions.findIndex(ver => ver.id === v.id) + 1} — {new Date(v.createdAt).toLocaleDateString()}</option>
                                        ))}
                                    </select>
                                    <RiHistoryLine size={12} className="text-zinc-400 absolute right-2 pointer-events-none" />
                                </div>
                                <div className="w-px h-4 bg-zinc-200 dark:bg-zinc-800 mx-1"></div>
                                <button onClick={handleCopy} className={`text-xs flex items-center gap-1.5 px-2.5 py-1 rounded-md transition-colors ${copied ? 'bg-green-50 text-green-700' : 'text-zinc-600 hover:text-zinc-900 hover:bg-zinc-100'}`}>
                                    {copied ? <RiCheckLine size={14} /> : <RiFileCopyLine size={14} />} <span>{copied ? 'Copied' : 'Copy'}</span>
                                </button>
                            </div>
                        )}
                    </div>
                    {isLocked ? (
                        <LockedPlaceholder title="Private Prompt" message="Please login to view content." />
                    ) : (
                        <div className="flex-1 group relative bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm overflow-hidden flex flex-col">
                            <div className="h-8 bg-zinc-50/50 dark:bg-zinc-800/30 border-b border-zinc-100 dark:border-zinc-800 flex items-center px-4 justify-between shrink-0">
                                <span className="text-xs font-bold text-zinc-400 uppercase tracking-widest">SOURCE</span>
                                <span className="text-xs font-mono text-zinc-400">{viewedVersion?.content.length || 0} CHARS</span>
                            </div>
                            <div className="flex-1 p-6 overflow-y-auto scrollbar-thin">
                                <pre className="font-mono text-sm leading-relaxed text-zinc-700 dark:text-zinc-300 whitespace-pre-wrap break-words">
      ...
      <nav className="lg:hidden fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50 animate-in fade-in slide-in-from-bottom-4 duration-300 pb-[env(safe-area-inset-bottom)]">
        <div className="bg-zinc-900/90 dark:bg-zinc-800/90 backdrop-blur-md text-white p-1.5 rounded-full flex shadow-2xl border border-white/10">
             <button onClick={() => setMobileTab('info')} className={`px-6 py-2.5 rounded-full text-xs font-medium flex items-center gap-2 transition-all ${mobileTab === 'info' ? 'bg-white text-black shadow-sm scale-105' : 'text-zinc-400 hover:text-white'}`}>
                <RiInformationLine size={16} /> Info
             </button>
             <button onClick={() => setMobileTab('prompt')} className={`px-6 py-2.5 rounded-full text-xs font-medium flex items-center gap-2 transition-all ${mobileTab === 'prompt' ? 'bg-white text-black shadow-sm scale-105' : 'text-zinc-400 hover:text-white'}`}>
                <RiFileTextLine size={16} /> Prompt
             </button>
        </div>
      </nav>
    </article>
